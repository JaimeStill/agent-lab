...
// and query execution. It offers generic, type-safe utilities that reduce boilerplate
// while keeping domain logic in the calling code.
package repository
...
// It allows query functions to work with any of these types.
...
// It allows exec functions to work with any of these types.
type Executor interface {
	ExecContext(ctx context.Context, query string, args ...any) (sql.Result, error)
}

// Scanner abstracts row scanning for use with query helpers.
// It is implemented by *sql.Row and *sql.Rows.
...
// It uses the provided scan function to convert each row into type T.
